...
public class UiCountryControllerSecurityTest {
    ...
    void testDelete_AccessGranted() throws Exception {
        doNothing().when(service).logicalDelete(contractorDto.getId());

        mockMvc.perform(delete("/ui/contractor/delete/" + contractorDto.getId())
    ...
    void testGetById_Unauthorized() throws Exception {
        mockMvc.perform(get("/ui/contractor/" + UUID.randomUUID()))
                .andExpect(status().isUnauthorized());
    }
